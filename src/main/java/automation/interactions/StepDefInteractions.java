...
/**
 * StepDefInteractions — Automation Layer
 * ----------------------------------------
 * Lives in the {@code automation} module, <em>not</em> in the core framework.
 * <p>
 * Provides high-level, reusable methods for Cucumber / BDD step definitions,
 * using the context-driven element-resolution strategy exposed by
 * {@link EnumClassRegistry}.
 * <p>
 * Inherits all low-level UI actions from the framework's {@link Interactions}
 * class. The automation layer owns the mapping from plain-text Cucumber
 * parameters (e.g. {@code "tiles"}, {@code "admin_home"}) to strongly-typed
 * element enums.
 *
 * <h3>Layer responsibilities</h3>
 * <pre>
 *  ┌──────────────────────────────────────────────────────────┐
 *  │  automation layer  (this package)                        │
 *  │  - StepDefInteractions  → BDD / Cucumber-specific logic  │
 *  │  - AutomationVOID       → automation façade              │
 *  │  - StepDefinition.*     → Cucumber @Step glue code       │
 *  ├──────────────────────────────────────────────────────────┤
 *  │  framework layer   (core / elements / interactions)      │
