...
public class SauceDemoTest {
    ...
    private LocatorDescriptor resolve(UIElement el, ElementRole role) {
        return engine().resolve(el, role);
    }

    private LocatorDescriptor resolve(UIElement el, ElementRole role, Object arg) {
        return engine().resolve(el, role, arg);
    }

    private String text(UIElement el, ElementRole role) {
        return engine().getText(resolve(el, role));
    }

    private String attr(UIElement el, ElementRole role, String attribute) {
        return engine().getAttribute(resolve(el, role), attribute);
    }
    ...
    private String loginErrorText() {
        return text(LoginPage.ErrorMessage.Labels.ERROR_BANNER, ElementRole.TEXT);
    }

    private String cartBadge() {
        return text(ProductsPage.Header.Labels.CART_BADGE, ElementRole.TEXT);
    }

    private void selectSortOption(String visibleText) {
        engine().selectByVisibleText(
                resolve(ProductsPage.SortDropdown.Controls.SORT_SELECT, ElementRole.TRIGGER),
                visibleText);
    }

    private void addItemToCart(String slug) {
        engine().click(resolve(ProductsPage.ProductItem.Buttons.ADD_TO_CART_BUTTON, ElementRole.TRIGGER, slug));
    }
    ...
    private List<String> productNames() {
        return engine().getAllTexts(resolve(ProductsPage.ProductItem.Labels.ITEM_NAME, ElementRole.TEXT));
    }

    private List<Double> productPrices() {
        return engine().getAllTexts(resolve(ProductsPage.ProductItem.Labels.ITEM_PRICE, ElementRole.TEXT))
                .stream()
    ...
    public void login10_passwordFieldMasked() {
        assertEquals(
                attr(LoginPage.LoginForm.Credentials.PASSWORD_FIELD, ElementRole.INPUT, "type"),
                "password");
    }
    ...
    public void login12_errorDismissal() {
        ...
        assertFalse(engine().isVisible(resolve(LoginPage.ErrorMessage.Labels.ERROR_BANNER, ElementRole.TEXT)),
    ...
    public void login13_loginButtonAlwaysEnabled() {
        String disabled = attr(LoginPage.LoginForm.Buttons.LOGIN_BUTTON, ElementRole.TRIGGER, "disabled");
    ...
    public void land01_productsPageLoads() {
        doValidLogin();
        assertTrue(app.getCurrentUrl().contains("/inventory.html"));
        assertEquals(text(ProductsPage.Header.Labels.PAGE_TITLE, ElementRole.TEXT), "Products");
        assertEquals(engine().getElementCount(resolve(ProductsPage.ProductItem.Labels.ITEM_NAME, ElementRole.TEXT)), 6,
    ...
    public void land02_defaultSortAtoZ() {
        doValidLogin();
        assertEquals(text(ProductsPage.SortDropdown.Labels.ACTIVE_OPTION_LABEL, ElementRole.TEXT),
                "Name (A to Z)");
    ...
    public void land06_addSingleItemToCart() {
        ...
        assertTrue(engine().isVisible(
                resolve(ProductsPage.ProductItem.Buttons.REMOVE_BUTTON, ElementRole.TRIGGER, "sauce-labs-backpack")),
    ...
    public void land08_removeItemFromProductsPage() {
        ...
        addItemToCart("sauce-labs-backpack");
        assertEquals(cartBadge(), "1");
        engine().click(resolve(ProductsPage.ProductItem.Buttons.REMOVE_BUTTON, ElementRole.TRIGGER, "sauce-labs-backpack"));
        assertFalse(engine().isVisible(resolve(ProductsPage.Header.Labels.CART_BADGE, ElementRole.TEXT)),
    ...
    public void land09_cartIconNavigation() {
        ...
        assertTrue(engine().getElementCount(resolve(CartPage.CartItem.Labels.ITEM_NAME, ElementRole.TEXT)) > 0,
    ...
    public void land11_hamburgerMenuOpens() {
        doValidLogin();
        openHamburgerMenu();
        assertTrue(engine().isVisible(resolve(ProductsPage.SideMenu.ALL_ITEMS_LINK, ElementRole.TRIGGER)),
                "All Items link should be visible");
        assertTrue(engine().isVisible(resolve(ProductsPage.SideMenu.ABOUT_LINK, ElementRole.TRIGGER)),
                "About link should be visible");
        assertTrue(engine().isVisible(resolve(ProductsPage.SideMenu.LOGOUT_LINK, ElementRole.TRIGGER)),
                "Logout link should be visible");
        assertTrue(engine().isVisible(resolve(ProductsPage.SideMenu.RESET_APP_STATE_LINK, ElementRole.TRIGGER)),
    ...
    public void land13_closeHamburgerMenu() {
        ...
        assertFalse(engine().isVisible(resolve(ProductsPage.SideMenu.CLOSE_MENU_BUTTON, ElementRole.TRIGGER)),
    ...
    public void land14_footerTwitterLink() {
        doValidLogin();
        assertEquals(
                attr(ProductsPage.Footer.TWITTER_LINK, ElementRole.TRIGGER, "href"),
    ...
    public void land15_footerFacebookLink() {
        doValidLogin();
        assertEquals(
                attr(ProductsPage.Footer.FACEBOOK_LINK, ElementRole.TRIGGER, "href"),
    ...
    public void land16_footerLinkedInLink() {
        doValidLogin();
        assertEquals(
                attr(ProductsPage.Footer.LINKEDIN_LINK, ElementRole.TRIGGER, "href"),
    ...
    public void cart01_cartShowsCorrectItem() {
        ...
        goToCart();
        assertTrue(app.getCurrentUrl().contains("/cart.html"));
        assertTrue(engine().getText(resolve(CartPage.CartItem.Labels.ITEM_NAME, ElementRole.TEXT)).contains("Onesie"),
                "Cart should display the Onesie item");
        assertEquals(engine().getText(resolve(CartPage.CartItem.Labels.ITEM_PRICE, ElementRole.TEXT)), "$7.99");
    }
    ...
    public void cart03_checkoutEmptyFirstName() {
        ...
        assertEquals(
                text(CheckoutStepOnePage.ErrorMessage.ERROR_BANNER, ElementRole.TEXT),
    ...
    public void cart07_checkoutOverviewTotals() {
        ...
        double subtotal = Double.parseDouble(
                text(CheckoutStepTwoPage.OrderSummary.PriceTotal.SUBTOTAL_LABEL, ElementRole.TEXT)
                        .replaceAll("[^0-9.]", ""));
        double tax = Double.parseDouble(
                text(CheckoutStepTwoPage.OrderSummary.PriceTotal.TAX_LABEL, ElementRole.TEXT)
                        .replaceAll("[^0-9.]", ""));
        double total = Double.parseDouble(
                text(CheckoutStepTwoPage.OrderSummary.PriceTotal.TOTAL_LABEL, ElementRole.TEXT)
                        .replaceAll("[^0-9.]", ""));
    ...
    public void cart09_backHomeAfterOrder() {
        ...
        goToCart();
        startCheckout();
        fillCheckoutInfo("Jane", "Smith", "99999");
        app.run(Flow.of(
    ...
    public void cart12_removeItemFromCartPage() {
        ...
        addItemToCart("sauce-labs-backpack");
        goToCart();
        engine().click(resolve(CartPage.CartItem.Buttons.REMOVE_BUTTON, ElementRole.TRIGGER, "sauce-labs-backpack"));

        assertEquals(engine().getElementCount(resolve(CartPage.CartItem.Labels.ITEM_NAME, ElementRole.TEXT)), 0,
    ...
    public void cart13_emptyCartCheckout() {
        ...
        startCheckout();
        fillCheckoutInfo("Test", "User", "00000");
        app.run(Flow.of(CheckoutStepTwoPage.Actions.FINISH_BUTTON.click()));
    ...
    @Test(enabled = false,
          description = "CART-14 -- Generate PDF button (data-test='generate-pdf') absent on live SauceDemo")
    public void cart14_generatePdfEmptyCart() {
        // Disabled: CheckoutCompletePage has no Generate PDF button on the live environment.
        // The locator //button[@data-test='generate-pdf'] times out waiting for presence.
    }

    @Test(enabled = false,
          description = "CART-15 -- Generate PDF button (data-test='generate-pdf') absent on live SauceDemo")
    public void cart15_generatePdfMultipleItems() {
        // Disabled: same reason as CART-14.
    }
}
