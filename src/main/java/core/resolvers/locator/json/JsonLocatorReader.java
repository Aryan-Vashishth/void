...
public final class JsonLocatorReader {
    ...
    // Add this inside your JsonLocatorReader class
    public static String getRaw(String jsonFileName, String key, Object... args) {
        if (jsonFileName == null || jsonFileName.isBlank()) {
            throw new IllegalArgumentException("JSON file name is null/blank.");
        }

        // Normalize path (adapt folder name if yours differs)
        String base = jsonFileName.trim();
        if (!base.toLowerCase().endsWith(".json")) {
            int dot = base.lastIndexOf('.');
            base = (dot > 0) ? base.substring(0, dot) + ".json" : base + ".json";
        }
        String classpath = base.startsWith("locators/json/") ? base : "locators/json/" + base;

        // Load JSON from classpath
        try (InputStream in = Thread.currentThread().getContextClassLoader().getResourceAsStream(classpath)) {
            if (in == null) {
                throw new IllegalStateException("JSON file not found on classpath: " + classpath);
            }

            com.fasterxml.jackson.databind.ObjectMapper mapper = new com.fasterxml.jackson.databind.ObjectMapper();
            com.fasterxml.jackson.databind.JsonNode root = mapper.readTree(in);

            // Traverse recursively to find the field
            java.util.ArrayDeque<com.fasterxml.jackson.databind.JsonNode> stack = new java.util.ArrayDeque<>();
            stack.push(root);
            while (!stack.isEmpty()) {
                com.fasterxml.jackson.databind.JsonNode cur = stack.pop();
                if (cur.has(key)) {
                    String template = cur.get(key).asText();
                    return applyArgs(template, args);
                }
                cur.elements().forEachRemaining(stack::push);
            }
            throw new IllegalStateException("Missing locator key '" + key + "' in " + classpath);
        } catch (Exception e) {
            throw new IllegalStateException("Failed to load or parse JSON: " + classpath, e);
        }
    }
