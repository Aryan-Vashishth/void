...
public final class ConfigLoader {
    ...
    public static Properties loadFromClasspath(String resourcePath, ClasspathScope scope) {
        ...
        if (resourcePath == null || resourcePath.isBlank()) { CustomLogger.warn.log("[cp] blank path"); return p; }
        long start = System.nanoTime();
        try {
            ClassLoader cl = Thread.currentThread().getContextClassLoader();
            CustomLogger.debug.log("[cp:start] path=" + resourcePath + " scope=" + scope);
            if (scope == ClasspathScope.ANY) {
                try (InputStream in = cl.getResourceAsStream(resourcePath)) {
                    if (in != null) { p.load(in); CustomLogger.debug.log("[cp:loaded] ANY keys=" + p.size()); }
                    else CustomLogger.warn.log("[cp:miss] ANY path=" + resourcePath);
                }
                return p;
            }
            ...
            if (chosen != null) { try (InputStream in = chosen.openStream()) { p.load(in); } CustomLogger.debug.log("[cp:loaded] scope=" + scope + " matches=" + all.size() + " keys=" + p.size()); }
            else CustomLogger.warn.log("[cp:miss] scope=" + scope + " path=" + resourcePath);
        } catch (Exception e) {
            CustomLogger.error.failed("[cp:error] path=" + resourcePath + " scope=" + scope + " msg=" + e.getMessage());
        } finally {
            CustomLogger.debug.log("[cp:done] path=" + resourcePath + " ms=" + (System.nanoTime()-start)/1_000_000L);
        }
        return p;
    }
    ...
    public static Properties loadFromFile(Path path) {
        ...
        try {
            if (path != null && Files.exists(path)) { try (InputStream in = Files.newInputStream(path)) { p.load(in); } CustomLogger.debug.log("[file:loaded] path=" + path + " keys=" + p.size()); }
            else CustomLogger.warn.log("[file:miss] path=" + path);
        } catch (Exception e) {
            CustomLogger.error.failed("[file:error] path=" + path + " msg=" + e.getMessage());
        } finally { CustomLogger.debug.log("[file:done] path=" + path + " ms=" + (System.nanoTime()-start)/1_000_000L); }
        return p;
    }
    ...
    public static void writeToFile(Properties p, Path path, String comment) {
        ...
        try {
            if (path.getParent()!=null && !Files.exists(path.getParent())) { Files.createDirectories(path.getParent()); CustomLogger.debug.log("[write:mkdir] parent=" + path.getParent()); }
            ...
            CustomLogger.debug.log("[write:ok] path=" + path + " keys=" + src.size());
        } catch(Exception e){ CustomLogger.error.failed("[write:error] path=" + path + " msg=" + e.getMessage()); throw new IllegalStateException("Failed write: " + path,e);}
        finally { CustomLogger.debug.log("[write:done] path=" + path + " ms=" + (System.nanoTime()-start)/1_000_000L); }
    }

    // ================= Merge =================
    public static Properties merge(List<Properties> sources) {
        Properties out = new Properties(); int idx=0;
        for (Properties src : sources) { if (src==null){ CustomLogger.debug.log("[merge:skip] idx="+idx); idx++; continue; }
            ...
            CustomLogger.debug.log("[merge:layer] idx="+idx+" added="+(out.size()-before)+" total="+out.size()); idx++; }
        CustomLogger.debug.log("[merge:done] layers="+sources.size()+" finalKeys="+out.size()); return out; }
    ...
    public static synchronized void setActive(Properties p){ int prev=ACTIVE.size(); ACTIVE.clear(); if(p!=null) for(String k: p.stringPropertyNames()){ String v=p.getProperty(k); if(v!=null) ACTIVE.setProperty(k,v);} CustomLogger.debug.log("[active:set] prev="+prev+" now="+ACTIVE.size()); }
    public static String get(String key){ String v=get(key,null); if(v==null){ CustomLogger.error.failed("[get:miss] key="+key); throw new IllegalArgumentException("Missing config: "+key);} return v; }
    public static String get(String key, String def){ if(key==null||key.isBlank()){ CustomLogger.warn.log("[get] blank key"); return def; }
        ...
        CustomLogger.debug.log("[get] key="+key+" src="+src+" val="+v); return v==null?def:v.trim(); }
    public static Properties cloneOf(Properties p){ Properties c=new Properties(); if(p!=null) for(String k: p.stringPropertyNames()){ String v=p.getProperty(k); if(v!=null) c.setProperty(k,v);} CustomLogger.debug.log("[clone] size="+c.size()); return c; }
    ...
    public static Properties environmentAsProperties(Map<String,String> explicit, BiFunction<String,String,String> mapper){ Properties out=new Properties(); Map<String,String> env=System.getenv(); int ex=0,map=0;
        ...
        CustomLogger.debug.log("[env] explicit="+ex+" mapped="+map+" total="+out.size()); return out; }
    ...
    public static final class Layered {
        ...
        public static final class Builder {
            ...
            private Path resolveExternal(String sp,String ek){ String viaSys= sp==null?null:System.getProperty(sp); String viaEnv= ek==null?null:System.getenv(ek); String first=firstNonBlank(viaSys, viaEnv); if(first==null||first.isBlank()) return null; Path p=Paths.get(first); if(!Files.exists(p)) CustomLogger.warn.log("[ext:miss] path="+p); return p; }
        }
    }

    // ================= Temp =================
    public static Properties tempLoadOnly(Path p){ return loadFromFile(p); }
    public static Properties tempOverlay(Properties base, Path p){ Properties r=merge(base==null?new Properties():base, tempLoadOnly(p)); CustomLogger.debug.log("[temp:overlay] base="+(base==null?0:base.size())+" over="+tempLoadOnly(p).size()+" result="+r.size()); return r; }
    public static String tempPull(Path p,String k){ String v=tempLoadOnly(p).getProperty(k); CustomLogger.debug.log("[temp:pull] path="+p+" key="+k+" val="+v); return v; }
    public static Properties tempPullAll(Path p){ Properties all=tempLoadOnly(p); CustomLogger.debug.log("[temp:all] path="+p+" keys="+all.size()); return all; }
    public static synchronized void tempPush(Path p,String k,String v){ Objects.requireNonNull(p); Objects.requireNonNull(k); Properties t=tempLoadOnly(p); if(v==null) t.remove(k); else t.setProperty(k,v); writeToFile(t,p,"temp"); CustomLogger.debug.log("[temp:push] path="+p+" key="+k+" size="+t.size()); }
    public static synchronized void tempPushAll(Path p,Properties u,boolean overwrite){ Objects.requireNonNull(p); if(u==null) return; Properties w=overwrite?cloneOf(u):merge(tempLoadOnly(p),u); writeToFile(w,p,overwrite?"temp overwrite":"temp merge"); CustomLogger.debug.log("[temp:pushAll] path="+p+" overwrite="+overwrite+" size="+w.size()); }
    public static synchronized void tempDeleteKey(Path p,String k){ Objects.requireNonNull(p); Properties t=tempLoadOnly(p); if(t.containsKey(k)){ t.remove(k); writeToFile(t,p,"temp del"); CustomLogger.debug.log("[temp:del] path="+p+" key="+k); } else CustomLogger.warn.log("[temp:del] missing key="+k+" path="+p); }
    public static synchronized void tempClear(Path p){ Objects.requireNonNull(p); writeToFile(new Properties(),p,"temp clear"); CustomLogger.debug.log("[temp:clear] path="+p); }

    // ================= Template =================
    public static Path createTemplate(Path path, Collection<String> keys, Properties examples, boolean header, boolean overwrite){ Objects.requireNonNull(path); try { if(!overwrite && Files.exists(path)){ CustomLogger.warn.log("[tpl:exists] path="+path); throw new IllegalStateException("Exists: "+path); }
            ...
            Files.writeString(path,sb.toString(),StandardCharsets.UTF_8,StandardOpenOption.CREATE,StandardOpenOption.TRUNCATE_EXISTING); CustomLogger.debug.log("[tpl:ok] path="+path+" keys="+wrote); return path; }
        catch(Exception e){ CustomLogger.error.failed("[tpl:error] path="+path+" msg="+e.getMessage()); throw new IllegalStateException("Template fail: "+path,e);} }
