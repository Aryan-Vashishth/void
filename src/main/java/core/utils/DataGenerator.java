...
import static core.utils.json.JsonLogger.Write.MapWriter.writeFlatMap;
...
public class DataGenerator extends BaseUtils {
    ...
    public static void saveFieldTypeSamples(String relativePath) {
        ...
        writeFlatMap(null, relativePath, samples);
    }
    ...
    public static void saveFieldTypeMapAsJson(Map<String, FieldType> fieldTypeMap, String relativePath) {
        if (fieldTypeMap == null || fieldTypeMap.isEmpty()) {
            warn.log("FieldType map is null or empty. Skipping save.");
            return;
        }
        Map<String, String> stringified = new LinkedHashMap<>();
        for (Map.Entry<String, FieldType> entry : fieldTypeMap.entrySet()) {
            stringified.put(entry.getKey(), (entry.getValue() == null ? "null" : entry.getValue().name()));
        }
        writeFlatMap(null, relativePath, stringified);
        info.log("FieldType map saved to JSON: " + relativePath);
    }
