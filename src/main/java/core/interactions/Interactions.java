...
public class Interactions {
    ...
    /**
     * Sets a {@link Checkbox} to the desired state (checked / unchecked).
     * <p>
     * Reads the current state via {@code aria-checked}, {@code checked} attribute,
     * or {@link WebElement#isSelected()}, then clicks only if a toggle is needed.
     *
     * @param checkbox     the checkbox element descriptor
     * @param desiredState {@code true} to check, {@code false} to uncheck
     * @throws RuntimeException if the checkbox cannot be located or toggled
     */
    ...
    /**
     * Checks whether <em>any</em> element matching the given locator is currently displayed,
     * polling for up to {@code timeout}.
     *
     * @param locator the {@link By} to search for
     * @param timeout maximum time to wait
     * @param poll    polling interval
     * @return {@code true} if at least one matching element is displayed within the timeout
     */
    ...
    /**
     * Convenience overload with sensible defaults (5 s timeout, 150 ms poll).
     *
     * @param locator the {@link By} to search for
     * @return {@code true} if at least one matching element is displayed
     */
    public boolean isAnyDisplayed(By locator) {
        return isAnyDisplayed(locator, Duration.ofSeconds(5), Duration.ofMillis(150));
    }
