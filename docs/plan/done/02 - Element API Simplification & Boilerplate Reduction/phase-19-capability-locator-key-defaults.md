...
**Status:** Complete — Part A (bare `name()` defaults) and Part B (fully-qualified key defaults + uniform nested JSON output) both shipped  
**Branch:** `feature/element-api-simplification`  
**Risk:** Medium — Part B changed the properties key format; required resolver update and migration of existing `.properties` files
...
## What Shipped (Phase 19 Part A — bare `name()` defaults)
...
## What Shipped (Phase 19 Part B — fully-qualified key defaults + uniform nested output)

Locator methods now default to the fully-qualified key with the `ElementRole` suffix appended. The default implementation derives it from the enum's class hierarchy via `Element.qualifiedLocatorKey()`:
...
### What landed in Part B

These changes landed together in commit `a6a111f`:

1. **`Element.qualifiedLocatorKey(Element, ElementRole)`** — new static helper; builds `PageClass.EnumClass.CONSTANT.ROLE` from the Java type hierarchy.
2. **Capability interfaces** (`Clickable`, `Typeable`, `ReadOnly`, `Listable`, `Uploadable`) — defaults updated from `name()` to `qualifiedLocatorKey(...)`. `getDisplayText()` fallback changed to `Element.super.getDisplayText()` (word-split enum name) to avoid the qualified key appearing in logs.
3. **`EnumLocatorScanner.writeInto()`** — the `roles.size() > 1` branch replaced with always-nested output; every element emits `{ "CONSTANT": { "ROLE": "value" } }`.
4. **Resource migration** — four locator data files migrated to the new format:
